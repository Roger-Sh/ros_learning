...
#### 3-1.action 实现方案A\(C++\) {#3-1action-实现方案ac}
...
//处理请求
void doReq(const demo02_test_action::turtle_moveGoalConstPtr
&
 goal, Server* s){
    ROS_INFO("目标值:x=%.2f, y=%.2f, theta=%.2f",goal-
>
turtle_goal_x,goal-
>
turtle_goal_y,goal-
...
void doPose(const turtlesim::Pose::ConstPtr
&
 pose){
    current_pose.x = pose-
>
...
("/turtle1/cmd_vel",1000);
    ...
    //boost::function
<
void (const demo02_test_action::turtle_moveGoalConstPtr 
&
)
>
 execute_callback
    Server server(nh,"turtle_move",boost::bind(
&
doReq,_1,
&
server), false);
...
#
include
"demo02_test_action/turtle_moveAction.h"
typedef
...
(
const
 demo02_test_action::turtle_moveFeedbackConstPtr 
&
feedback)
{
    ROS_INFO(
...
const
 demo02_test_action::turtle_moveResultConstPtr 
&
result)
{
    ROS_INFO(
...
// 5.发送请求
//void sendGoal(const demo02_test_action::turtle_moveGoal 
&
goal, 
//              boost::function
<
void (const actionlib::SimpleClientGoalState 
&
state, const demo02_test_action::turtle_moveResultConstPtr 
&
result)
>
 done_cb, 
...
//              boost::function
<
void (const demo02_test_action::turtle_moveFeedbackConstPtr 
&
feedback)
>
 feedback_cb)
...
#### 3-2.action 实现方案B\(Python\) {#3-2action-实现方案bpython}
...
from
 demo02_test_action.msg 
import
 turtle_moveAction, turtle_moveGoal, turtle_moveResult, turtle_moveFeedback
...
"/turtle1/pose"
,Pose,self.doPose,queue_size=
1000
)
        self.server = SimpleActionServer(topic,turtle_moveAction,self.exeCb,
False
)
        self.server.start()
        rospy.loginfo(
...
# 计算并发布速度信息
                ...
                jianju = math.sqrt(math.pow(goal_x - self.current_x,
2
) + math.pow(goal_y - self.current_y,
2
))
                msg.linear.x = 
0.5
...
0
                ...
                msg.angular.z = 
2
 * (math.atan2(goal_y - self.current_y, goal_x - self.current_x) - self.current_z);
...
from
 demo02_test_action.msg 
import
 turtle_moveAction, turtle_moveFeedback, turtle_moveGoal, turtle_moveResult
...
# 5.发送请求
    ...
    goal.turtle_goal_x = rospy.get_param(
"x"
)
    goal.turtle_goal_y = rospy.get_param(
"y"
)
    goal.turtle_goal_z = rospy.get_param(
"z"
)
